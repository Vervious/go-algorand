...
// stageDigestAction notifies the ledger that we have a certificate, but not a block.
// For now, the ledger asynchronously tries to do an ensureDigest.
...
func (a stageDigestAction) String() string {
	return fmt.Sprintf("%v: %.5v", a.t().String(), a.Certificate.Proposal.BlockDigest.String())
}
...
func (a stageDigestAction) do(ctx context.Context, service *Service) {
	...
	go service.Ledger.EnsureDigest(a.Certificate, service.quit, service.voteVerifier)
}
